...
//   const keypairPath = process.env.MY_WALLET_PATH || path.join(os.homedir(), '.config', 'solana', 'id.json');
//   const keypair = anchor.web3.Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(keypairPath, 'utf8'))));

//   const connection = new anchor.web3.Connection(anchor.web3.clusterApiUrl('devnet'), 'confirmed');

//   const programId = new anchor.web3.PublicKey(process.env.MY_PROGRAM_ID);
//   const idl = await anchor.Program.fetchIdl(programId, { connection });

//   const program = new anchor.Program(idl, programId, { wallet: new anchor.Wallet(keypair), connection });
